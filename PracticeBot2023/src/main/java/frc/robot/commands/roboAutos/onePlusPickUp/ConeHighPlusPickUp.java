...
public class ConeHighPlusPickUp extends SequentialCommandGroup {
  ...
  frc.robot.utils.PIDController straightHeadingController, turnController, driveStraightController;
  public ConeHighPlusPickUp(Chassis chassis, Gyro gyro, Elevator elevator, Intake intake) {
    ...
    turnController = new PIDController(.005, .0085, 0, 90, .25, .25, 0);//setting the target on turn controllers will not matter as they will be reset via constructor
    driveStraightController = new PIDController(.013, .01, 0, 35, 1, 1, Constants.Values.TICKS_PER_METER*.21);

    addCommands(
      new ConeHighPlusMobility(chassis, gyro, elevator, intake)
      ,new OpenClaw(intake)
      ,new Turn(chassis, gyro, 198, turnController, true)
